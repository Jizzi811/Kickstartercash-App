...
# ---------------------------------------------------------------------------
# Sprint C – Onboarding status + Brand Readiness
# ---------------------------------------------------------------------------
...
def _onboarding_scope(user_id: str, ws: Optional[str]) -> dict:
    return {"user_id": user_id, "workspace_id": ws or ""}
...
async def onboarding_status_update(payload: OnboardingStatusUpdate, user: dict = Depends(_authed_user), ws: Optional[str] = Depends(current_workspace)):
    now = _now_iso()
    existing = await db.onboarding_status.find_one(_onboarding_scope(user["id"], ws), {"_id": 0}) or {}
    update = {"updated_at": now}
    selected_path = payload.selected_path if payload.selected_path is not None else existing.get("selected_path", "")
    status = payload.status if payload.status is not None else existing.get("status", "not_started")
    if selected_path and selected_path not in ONBOARDING_PATHS:
        raise HTTPException(status_code=400, detail="Invalid selected_path")
    if status not in ONBOARDING_STATUSES:
        raise HTTPException(status_code=400, detail="Invalid onboarding status")
    update["status"] = status
    update["selected_path"] = selected_path or ""
    if payload.current_step is not None:
        update["current_step"] = payload.current_step.strip()[:80]
    elif "current_step" not in existing and selected_path:
        update["current_step"] = ONBOARDING_STEPS.get(selected_path, [""])[0]
    if payload.completed_steps is not None:
        allowed = set(ONBOARDING_STEPS.get(selected_path, [])) if selected_path else set()
        update["completed_steps"] = [str(x)[:80] for x in payload.completed_steps if (not allowed or x in allowed)][:30]
    if status in {"path_selected", "in_progress"} and not existing.get("started_at"):
...
def _has_real_value(value: Any) -> bool:
    ...
    if isinstance(value, list): return any(_has_real_value(v) for v in value)
    if isinstance(value, dict): return any(_has_real_value(v) for v in value.values())
    return bool(value)
...
def calculate_brand_readiness(brand: Optional[dict], knowledge_count: int, workspace_id: str) -> dict:
    ...
    for cid, de, en, pts, checks, route, action_de, action_en in criteria:
        present=[k for k,v in checks if _has_real_value(v)]; missing=[k for k,v in checks if not _has_real_value(v)]
...
@api_router.get("/brand-readiness")
async def brand_readiness(brand_id: Optional[str] = None, ws: Optional[str] = Depends(current_workspace)):
    if db is None:
        raise HTTPException(status_code=503, detail="Database not available")
    brand = await _resolve_brand(brand_id, ws)
    if not brand:
        return calculate_brand_readiness(None, 0, ws or "")
    kb_filter = dict(_scope_filter(ws)); kb_filter["brand_id"] = {"$in": [brand.get("id"), "", None]}
    knowledge_count = await db.knowledge.count_documents(kb_filter)
    return calculate_brand_readiness(brand, knowledge_count, ws or "")
