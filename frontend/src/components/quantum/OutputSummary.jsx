...
export default function OutputSummary({ workflow }) {
  ...
  return (
    <div className="min-w-0 overflow-hidden rounded-sm border border-white/8 bg-black/35 p-4">
      ...
      {outputs.length === 0 ? <div className="text-xs text-zinc-500">Noch keine Agenten-Ergebnisse im Output Bus.</div> : <div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2">{outputs.map((output) => <div key={output.id} className="rounded-sm border border-white/8 bg-white/[0.02] p-3"><div className="mb-1 text-xs font-semibold text-white">{output.title}</div><div className="text-xs text-zinc-400">{output.summary}</div></div>)}</div>}
    </div>
  );
}
