...
export default function FinanceTaxStudio() {
  ...
  const runTool = async (tool) => {
    ...
    try {
      const res = await axios.post(`${API}/agents/tools/run`, {
        agent_id: "tax", tool_id: tool.id, context, model: "gpt", language: lang, brand_id: activeBrandId,
      });
      if (res.data.reply) {
  ...
  return (
    <div className="min-w-0 space-y-5 pb-6 sm:space-y-8">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-4 lg:grid-cols-5 lg:gap-6">
        <div className="min-w-0 space-y-4 lg:col-span-3 lg:space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2 min-[380px]:grid-cols-2 sm:gap-3 xl:grid-cols-3">
              {TOOLS.map((tool) => (
        ...
        <div className="flex h-[min(560px,calc(100vh-7rem))] min-h-[420px] min-w-0 flex-col rounded-sm border border-white/8 bg-[#0A0A0A] lg:col-span-2 lg:h-auto lg:min-h-[520px]">
