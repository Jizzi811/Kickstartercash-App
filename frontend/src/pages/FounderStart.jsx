...
const COPY = {
  DE: { title: "Wo stehst du gerade?", sub: "Du musst noch keinen perfekten Plan haben. Sag uns einfach, wie weit du aktuell bist.", cta: "Weiter zum Gründerprofil", resume: "Speichern und später fortsetzen", savedNotice: "Du hast bereits Angaben gespeichert. Sie bleiben erhalten. Wähle bitte, wo du aktuell mit deiner Geschäftsidee stehst.", options: [
  ...
  EN: { title: "Where are you right now?", sub: "You do not need a perfect plan yet. Just tell us how far along you are.", cta: "Continue to founder profile", resume: "Save and continue later", savedNotice: "You already have saved information. It will stay in place. Please choose where you currently are with your business idea.", options: [
...
export default function FounderStart() {
  ...
  useEffect(() => { let alive = true; (async () => { try { const { data } = await axios.get(`${API}/founder/profile`); if (alive) { setSelected(data.founder_path || ""); setHasSavedData(["vision", "motivation", "target_audience", "selected_idea_id", "selected_idea", "idea_working_title", "idea_description", "rough_topic", "rough_thoughts"].some((key) => Boolean(data[key]))); } } catch { toast.error("Founder-Start konnte nicht geladen werden"); } finally { if (alive) setLoading(false); } })(); return () => { alive = false; }; }, []);
  ...
  return <Page><FounderProgress current="start" /><Hero icon={Compass} badge="Founder Entry" title={copy.title} description={copy.sub} actions={<Btn onClick={() => save(true)} disabled={!selected || saving || loading}>{saving ? <Loader2 size={14} className="animate-spin" /> : <ArrowRight size={14} />}{copy.cta}</Btn>} />
    {hasSavedData && <Card><p className="text-sm text-amber-200">{copy.savedNotice}</p></Card>}
