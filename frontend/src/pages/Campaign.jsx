...
export default function Campaign() {
  ...
  return (
    <div>
      ...
      <div className="bg-[#0A0A0A] border border-[#D4AF37]/30 rounded-md p-6 md:p-7 space-y-5 gold-glow">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {SOCIAL_PLATFORMS.map((p) => (
              <button key={p} data-testid={`campaign-platform-${p}`} onClick={() => toggle(p)}
                className={`px-3.5 py-1.5 rounded-sm text-sm border transition-all ${selected.includes(p) ? "bg-[#D4AF37] text-black border-[#D4AF37] font-semibold" : "border-white/10 text-zinc-400 hover:border-[#D4AF37]/40"}`}>
                {p}
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {IMAGE_STYLES.slice(0, 8).map((s) => (
              <button key={s} data-testid={`campaign-style-${s}`} onClick={() => setStyle(s)}
                className={`px-3 py-1.5 rounded-sm text-sm border transition-all ${style === s ? "bg-[#D4AF37] text-black border-[#D4AF37] font-semibold" : "border-white/10 text-zinc-400 hover:border-[#D4AF37]/40"}`}>
                {s}
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-4 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-zinc-600 ml-2">{t("activeBrand")}: <span className="text-[#D4AF37]">{activeBrand?.name}</span></span>
          </div>
      ...
      {!loading && result && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-8" data-testid="campaign-result">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0A0A0A] border border-white/10 rounded-md p-4 flex flex-col">
            <div className="text-xs tracking-[0.12em] uppercase text-[#D4AF37] mb-3 flex items-center gap-1.5"><ImageIcon size={13} /> {t("campaign_image")}</div>
            ...
            {result.image && (
              <button data-testid="campaign-image-download" onClick={downloadImg} className="mt-3 inline-flex items-center justify-center gap-2 border border-[#D4AF37]/50 text-[#D4AF37] py-2 rounded-sm hover:bg-[#D4AF37]/10 text-sm">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="bg-[#0A0A0A] border border-white/10 rounded-md p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs tracking-[0.12em] uppercase text-[#D4AF37] flex items-center gap-1.5"><Megaphone size={13} /> {t("campaign_copy")}</span>
