  ...
  const runTool = async (tool) => {
    ...
    try {
      const res = await axios.post(`${API}/agents/tools/run`, {
        agent_id: "linkedin", tool_id: tool.id, context, model: "gpt", language: lang,
      });
      if (res.data.reply) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-5">
          <div className="border border-white/8 rounded-sm p-5 space-y-4" style={{ background: "rgba(10,102,194,0.03)" }}>
            <label className="block text-xs uppercase tracking-widest" style={{ color: COLOR }}>
              {lang === "DE" ? "Thema / Idee / Ziel" : "Topic / Idea / Goal"}
            </label>
            <textarea value={context} onChange={(e) => setContext(e.target.value)} rows={4}
              placeholder={lang === "DE"
                ? "z.B. Warum finanzielle Freiheit kein Zufall ist – mein Weg mit KickstarterCash…"
                : "e.g. Why financial freedom is not a coincidence – my journey with KickstarterCash…"}
