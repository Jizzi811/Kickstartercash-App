  ...
  const runTool = async (tool) => {
    ...
    try {
      const res = await axios.post(`${API}/agents/tools/run`, {
        agent_id: "bookkeeper", tool_id: tool.id, context, model: "gpt", language: lang,
      });
      if (res.data.reply) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-5">
          <div className="border border-white/8 rounded-sm p-5 space-y-4" style={{ background: "rgba(5,150,105,0.03)" }}>
            <label className="block text-xs uppercase tracking-widest" style={{ color: COLOR }}>
              {lang === "DE" ? "Unternehmen / Buchhaltungskontext" : "Company / Accounting Context"}
            </label>
            <textarea value={context} onChange={(e) => setContext(e.target.value)} rows={4}
              placeholder={lang === "DE"
                ? "z.B. GmbH, 15 Mitarbeiter, QuickBooks, monatlicher Abschluss, nächstes Audit in 3 Monaten…"
                : "e.g. LLC, 15 employees, QuickBooks, monthly close, next audit in 3 months…"}
